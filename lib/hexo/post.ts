...
class Post {
  ...
  create(
    data: PostData,
    replace: boolean | NodeJSLikeCallback<any>,
        ...
        obj = deepMerge(
          ...
          Object.fromEntries(
            Object.entries(data).filter(
              ([key, value]) => !preservedKeys.includes(key) && value != null
            )
          )
        );
  ...
  publish(
    data: PostData,
    replace: boolean,
    callback?: NodeJSLikeCallback<Result>
  );
  publish(
    data: PostData,
    replace?: boolean | NodeJSLikeCallback<Result>,
    callback?: NodeJSLikeCallback<Result>
  ) {
    if (!callback && typeof replace === 'function') {
      callback = replace;
  ...
  render(
    source: string,
    data: RenderData = {},
    callback?: NodeJSLikeCallback<never>
  ) {
    const ctx = this.context;
    const { config } = ctx;
